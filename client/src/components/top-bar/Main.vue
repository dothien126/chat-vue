<template>
  ...
  <div class="top-bar">
    ...
    <Dropdown class="intro-x w-8 h-8">
      ...
      >
        <img alt="Tinker Tailwind HTML Admin Template" v-bind:src="currentUser?.avatar" />
      </DropdownToggle>
      <DropdownMenu class="w-56">
        <DropdownContent class="bg-primary text-white">
          <DropdownHeader tag="div" class="!font-normal">
            <div class="font-medium">{{ currentUser.username }}</div>
            <div class="text-xs text-white/70 dark:text-slate-500 mt-2">Try to best</div>
          </DropdownHeader>
          <DropdownDivider class="border-white/[0.08]" />
          <DropdownItem class="dropdown-item hover:bg-white/5">
            <input type="file" />
            <button class="upload-button" :disabled="!this.selectedFile">Up Avatar</button>
          </DropdownItem>
