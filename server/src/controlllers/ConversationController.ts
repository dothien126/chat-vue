...
export const findListConversation = async function (req: Request, res: Response) {
  try {
    const authorization = req.headers['authorization'];
    if (!authorization) {
      return errJwtNotVerify(res);
...
export const markConversationBlockHandle = async (req: Request, res: Response) => {
  try {
    const authorization = req.headers['authorization'];
    if (!authorization) {
      return errJwtNotVerify(res);
    ...
    if (verify) {
      ...
      const itemService = await markConversationBlockServices(conversationId);
      return res.json(itemService);
    } else {
      return errJwtNotVerify(res);
    }
...
export const unBlockConversationHandle = async (req: Request, res: Response) => {
  try {
    const authorization = req.headers['authorization'];
    if (!authorization) {
      return errJwtNotVerify(res);
    ...
    if (verify) {
      const { conversationId } = req.body;
      const itemService = await unBlockConversationServices(conversationId);
      return res.json(itemService);
    } else {
      return errJwtNotVerify(res);
    }
